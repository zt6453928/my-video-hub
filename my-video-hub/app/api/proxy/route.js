// 文件路径: app/api/proxy/route.js (v4.1 - 缓存 TTL 优化版)
...
// 将 Web Stream 写入 Node.js 文件流的辅助函数 (优化为 Promise)
...
export async function GET(request) {
    ...
    try {
        ...
        console.log(`[Proxy Cache] STALE (过期): ${path.basename(targetUrl)}`);
    } catch (error) {
        // 缓存未命中 (文件不存在)
    ...
    try {
        const response = await fetch(targetUrl, { headers: { 'Referer': referer || new URL(targetUrl).origin, 'User-Agent': 'Mozilla/5.0' } });
        if (!response.ok) {
            return new Response(`无法获取目标资源: ${response.statusText}`, { status: response.status });
        }
        ...
        writeStreamToFile(streamToCache, cacheFilePath).catch(err => {
            console.error('[Proxy Cache] 异步写入缓存失败:', err);
            // 尝试清理不完整的文件
            fs.unlink(cacheFilePath).catch(e => console.error(`[Proxy Cache] 清理失败的缓存文件时出错: ${e.message}`));
        });
